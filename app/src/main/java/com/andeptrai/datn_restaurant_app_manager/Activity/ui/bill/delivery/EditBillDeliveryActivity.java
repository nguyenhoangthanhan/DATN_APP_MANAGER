...
import static com.andeptrai.datn_restaurant_app_manager.URL.urlUpdateBillDeliveryByIdBill;
...
public class EditBillDeliveryActivity extends AppCompatActivity implements View.OnClickListener, DeliveryFoodInterf{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //add 31 - 5
        txtChangeStatusConfirm = findViewById(R.id.txtChangeStatusConfirm);
    ...
    @Override
    public void onClick(View view) {
        if (view.getId() == R.id.txtChangeStatusConfirm){
            openChangeStatusConfirmDialog();
        }
        else if (view.getId() == R.id.txtChangeAddress){
    ...
    private void openChangeStatusConfirmDialog() {
        final Dialog dialog = new Dialog(this);
        dialog.setContentView(R.layout.dialog_change_status_confirm);

        final RadioGroup groupStatusConfirm = dialog.findViewById(R.id.groupStatusConfirm);
        Button btnAccept = dialog.findViewById(R.id.btnAccept);
        Button btnBack = dialog.findViewById(R.id.btnBack);

        if (currentBillDelivery.getStatusConfirm() == 1){
            groupStatusConfirm.check(R.id.acceptBill);
        }
        else if (currentBillDelivery.getStatusConfirm() == 2){
            groupStatusConfirm.check(R.id.rejectBill);
        }

        btnAccept.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (groupStatusConfirm.getCheckedRadioButtonId() == R.id.acceptBill){
                    currentBillDelivery.setStatusConfirm(1);
                    txtStatusConfirm.setText(R.string.accepted);
                }
                else if (groupStatusConfirm.getCheckedRadioButtonId() == R.id.rejectBill){
                    currentBillDelivery.setStatusConfirm(2);
                    txtStatusConfirm.setText(R.string.rejected);
                }
                dialog.dismiss();
            }
        });
